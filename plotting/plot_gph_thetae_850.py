...
def main():
    ...
    theta_e = mpcalc.equivalent_potential_temperature(850 * units.hPa, dset['t'].metpy.sel(vertical=850 * units.hPa),
                     mpcalc.dewpoint_from_relative_humidity(dset['t'].metpy.sel(vertical=850 * units.hPa), dset['r'].metpy.sel(vertical=850 * units.hPa)/100.)).to(units.degC)
    ...
    levels_mslp = np.arange(mslp.magnitude.min().astype("int"), mslp.magnitude.max().astype("int"), 2.5)
