...
def main():
    ...
    tmin2m = dset['TMIN_2M'].squeeze() - 273.15
    ...
    for projection in projections:# This works regardless if projections is either single value or array
        ...
        args=dict(m=m, x=x, y=y, ax=ax, cmap=cmap,
                 tmin2m=tmin2m, levels_t2m=levels_t2m,
...
def plot_files(dates, **args):
    ...
    for date in dates:
        ...
        cs = args['ax'].contourf(args['x'], args['y'], args['tmin2m'][i], extend='both', cmap=args['cmap'],
        ...
        vals = add_vals_on_map(args['ax'], args['m'], args['tmin2m'][i], args['levels_t2m'], cmap=args['cmap'],
                                density=density)

        an_fc = annotation_forecast(args['ax'],args['time'][i])
        an_var = annotation(args['ax'], 'Minimum 2m Temperature in last 6 hours' ,loc='lower left', fontsize=6)
